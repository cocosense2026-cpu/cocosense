...
import { toCamel, validateAvatarDataUrl, applyDerivedNodeHealth, PIEZO_PINS, piezoSensorId } from '../utils.js';
...
async function logActivity(superadminId, action, detail) {
  ...
  await restampRowHash(db, 'superadmin_activity', 'id', result.lastInsertRowid);
}
...
router.patch('/superadmin/profile', requireSuperAdminAuth, async (req, res) => {
  ...
  try {
    ...
    await restampRowHash(db, 'superadmins', 'id', req.superadminRow.id);
  } catch (err) {
...
router.post('/superadmin/admins', requireSuperAdminAuth, async (req, res) => {
  ...
  await restampRowHash(db, 'admins', 'id', id);
...
router.patch('/superadmin/admins/:id/status', requireSuperAdminAuth, async (req, res) => {
  ...
  await restampRowHash(db, 'admins', 'id', admin.id);
  if (status === 'Inactive') {
...
// ========== Master Node QR provisioning ==========
// Lets a super admin pre-provision physical master node hardware before
// it's ever handed to a farm owner: generate a fresh, unassigned node
// record (a real row in master_nodes, owner_id = NULL) with a stable ID,
// print a QR sticker for its housing, and -- once a farmer scans that
// sticker -- the Owner Portal's self-service signup form
// (src/owner/pages/SignupPage.tsx, POST /owner/auth/signup) links the
// new account straight to this exact node id instead of the admin
// having to hand-provision hardware afterward via "Expand Nodes".

// Stock nodes get their own id namespace ("MN-STOCK-0007") so they never
// collide with "MN-{ownerId}-NNN" ids issued once a node is already
// tied to an owner (see provisionMasterNodes in server/routes/api.js).
async function nextStockNodeId() {
  const prefix = 'MN-STOCK-';
  const rows = await db.prepare(`SELECT id FROM master_nodes WHERE id LIKE ?`).all(`${prefix}%`);
  let max = 0;
...
// Every stock node is provisioned with its full 4-sensor piezo array up
// front (A0-A3, same pin mapping as provisionMasterNodes) so the printed
// QR really does correspond to ready-to-ship hardware -- assigning it to
// an owner later is then just a single UPDATE, not a fresh provisioning
// step.
router.post('/superadmin/nodes', requireSuperAdminAuth, async (req, res) => {
  const sector = (req.body?.sector || '').trim() || null;
  const nodeId = await nextStockNodeId();
  ...
  await db.prepare(
    `INSERT INTO master_nodes
     ...
     VALUES (?,?,?,NULL,0,NULL,NULL,4,0,0,NULL,'v2.4.8-STABLE')`
  ).run(nodeId, `Master Node ${nodeId.slice('MN-STOCK-'.length)}`, sector);
  await restampRowHash(db, 'master_nodes', 'id', nodeId);

  // Sensors sit as NOT_CONNECTED (same status an unwired A3 pin already
  // uses elsewhere) until the node is actually assigned to an owner and
  // deployed in the field -- nothing to read from hardware that isn't
  // installed yet.
  for (const pin of PIEZO_PINS) {
    const sensorId = piezoSensorId(nodeId, pin);
    await db.prepare(
      `INSERT INTO piezo_sensors (id, node_id, status, frequency_hz, voltage_mv) VALUES (?,?,'NOT_CONNECTED',0,0)`
    ).run(sensorId, nodeId);
    await restampRowHash(db, 'piezo_sensors', 'id', sensorId);
  }

  await logActivity(req.superadminRow.id, 'Generated master node QR', nodeId);

  const node = toCamel(await db.prepare(`SELECT * FROM master_nodes WHERE id = ?`).get(nodeId));
  applyDerivedNodeHealth(node);
  node.coordinates = [node.lat, node.lng];
  node.sensors = toCamel(await db.prepare(`SELECT * FROM piezo_sensors WHERE node_id = ?`).all(nodeId));
