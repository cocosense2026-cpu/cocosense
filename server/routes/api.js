...
import { toCamel, PIEZO_PINS, piezoSensorId, applyDerivedNodeHealth, NODE_OFFLINE_AFTER_MS } from '../utils.js';
...
async function provisionMasterNodes(ownerId, sector, count) {
  // Base numbering on the highest node number ever issued under this
  // owner ID -- not just nodes currently assigned to them. DELETE
  // /owners/:id only unassigns hardware (sets owner_id = NULL), it
  // never deletes the master_nodes rows themselves (see that route's
  // comment for why). So if this owner ID is ever reused -- a new
  // owner registered with the same id after an old one was removed,
  // or nodes unassigned-then-reassigned -- counting only
  // *currently-assigned* nodes would restart numbering at 1 and
  // collide with those still-existing orphaned "MN-{ownerId}-NNN"
  // rows, throwing a UNIQUE constraint error. Scanning every row that
  // was ever issued this owner's id prefix (regardless of current
  // owner_id) avoids the collision no matter how the id got reused.
  const prefix = `MN-${ownerId}-`;
  const existingIds = await db.prepare(`SELECT id FROM master_nodes WHERE id LIKE ?`).all(`${prefix}%`);
  let maxNumber = 0;
  ...
  for (let i = 0; i < count; i++) {
    const nodeNumber = maxNumber + i + 1;
    const nodeId = `${prefix}${String(nodeNumber).padStart(3, '0')}`;
    await db.prepare(
      `INSERT INTO master_nodes
        (id, name, sector, owner_id, online, battery_percent, signal_rssi, total_sensors, working_sensors, damaged_sensors, last_ping, firmware_version, linked_at)
       VALUES (?,?,?,?,1,100,?,4,3,0,datetime('now'),'v2.4.8-STABLE',datetime('now'))`
...
router.get('/dashboard', async (req, res) => {
  ...
  const nodesOnline = (await db
    .prepare(
      `SELECT COUNT(*) AS n FROM master_nodes
       WHERE last_ping IS NOT NULL
         AND (strftime('%s','now') - strftime('%s', last_ping)) < ?`
    )
    .get(NODE_OFFLINE_AFTER_MS / 1000)).n;
...
router.delete('/owners/:id', async (req, res) => {
  ...
  try {
    ...
    await db.prepare(`UPDATE master_nodes SET owner_id = NULL, linked_at = NULL WHERE owner_id = ?`).run(req.params.id);
