...
// Same "where does the confirm link point" logic as server/routes/api.js
...
// self-registering owner has no admin around to hand out the next
// number, so this generates one server-side: scan every id ever issued
// under this year's prefix (not just currently-active ones) and take
// the highest sequence + 1, the same collision-safe approach
// provisionMasterNodes() in server/routes/api.js uses for node ids.
async function nextOwnerId() {
  const year = new Date().getFullYear();
  const prefix = `FO-${year}-`;
  const rows = await db.prepare(`SELECT id FROM farm_owners WHERE id LIKE ?`).all(`${prefix}%`);
  let max = 0;
...
const OWNER_SIGNUP_COLORS = ['#059669', '#0284c7', '#7c3aed', '#d97706', '#dc2626', '#4f46e5'];

function signupConfirmationEmail({ name, token }) {
  const link = `${OWNER_PORTAL_URL}/owner/confirm?token=${token}`;
  const body =
    `Hi ${name},\n\n` +
    `Thanks for creating a CocoSense Farm Owner account. Confirm your email to activate it:\n\n` +
    `  ${link}\n\n` +
    `This link expires in 24 hours. Once confirmed, sign in at ${OWNER_PORTAL_URL}/owner/login with the ` +
...
<div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#1A1A1A;">
  <h2 style="margin:0 0 12px;font-size:18px;">Welcome to CocoSense, ${name}.</h2>
  <p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#3A3A38;">Your Farm Owner account has been created. Confirm your email to activate it.</p>
  <a href="${link}" style="display:inline-block;padding:11px 22px;background:#16A34A;border-radius:6px;font-size:13px;font-weight:bold;color:#FFFFFF;text-decoration:none;">Confirm your account</a>
  <p style="margin:22px 0 0;font-size:12px;line-height:1.6;color:#6A6A66;">This link expires in 24 hours. If you didn't create this account, you can ignore this email.</p>
</div>`;
  return { body, html };
}
...
// Public, unauthenticated lookup for a master-node QR code -- called by
// the Create Account form (src/owner/pages/SignupPage.tsx) as soon as it
// loads with a ?nodeId= param, purely to show the prospective owner
// which hardware they're about to link (or a clear "already used" /
// "not recognized" message) before they fill anything in. Deliberately
// returns almost nothing about the node itself (no owner name, no
// telemetry) -- just enough to render that banner.
router.get('/owner/node-lookup/:id', async (req, res) => {
  const node = await db.prepare(`SELECT id, owner_id FROM master_nodes WHERE id = ?`).get(req.params.id);
  if (!node) return res.json({ ok: true, valid: false, reason: 'not_found' });
  if (node.owner_id) return res.json({ ok: true, valid: false, reason: 'already_linked' });
  res.json({ ok: true, valid: true, nodeId: node.id });
});

// Public self-registration -- independent of the admin console. A
// prospective owner fills this in themselves (no admin involvement to
// create the row), picks their own password up front, and confirms by
// email exactly like an admin-invited owner does. Because this inserts
// straight into farm_owners, the new owner shows up in the admin's Farm
// Owners list immediately (unconfirmed, 0 nodes/trees until an admin
// assigns hardware) -- same table, same list, no separate storage.
router.post('/owner/auth/signup', async (req, res) => {
  const b = req.body || {};
  ...
  // Address -- CocoSense only registers farms within Aurora province, so
  // the Create Account form's PSGC cascade (src/owner/pages/SignupPage.tsx)
  // always fixes country/region/province itself; City/Municipality and
  // Barangay are the only fields that actually vary, and are required.
  const country = (b.country || 'Philippines').trim();
  const region = (b.region || 'Region III (Central Luzon)').trim();
  const province = (b.province || 'Aurora').trim();
  const cityMunicipality = (b.cityMunicipality || '').trim();
  const barangay = (b.barangay || '').trim();
  const street = (b.street || '').trim() || null;
  const geoCoordinates = (b.geoCoordinates || '').trim() || null;
  // Set when this signup came from scanning a super-admin-printed
  // master node QR sticker (see server/routes/superadmin.js's
  // POST /superadmin/nodes and src/superadmin/pages/SuperAdminQrCodesPage.tsx).
  // Re-validated here server-side regardless of what the lookup endpoint
  // already told the client, since that's just a UI hint, not a trust
  // boundary.
  const masterNodeId = (b.masterNodeId || '').trim() || null;
  const address =
    (b.address || '').trim() ||
    (cityMunicipality && barangay
      ? `${street || 'Main Farm Compound'}, ${barangay}, ${cityMunicipality}, ${province}, ${country}`
      : null);
  ...
  if (!cityMunicipality || !barangay) {
    return res.status(400).json({ ok: false, error: 'Please select your city/municipality and barangay.' });
  }
  const strengthError = passwordStrengthError(password);
  if (strengthError) return res.status(400).json({ ok: false, error: strengthError });

  const existing = await db.prepare(`SELECT id FROM farm_owners WHERE LOWER(TRIM(email)) = LOWER(TRIM(?))`).get(email);
  if (existing) {
    return res.status(409).json({ ok: false, error: 'An account with this email already exists. Try signing in instead.' });
  }

  // Same re-check as /owner/node-lookup/:id -- fail the whole signup
  // before creating any row if the QR-scanned node turns out to be
  // invalid or was claimed by someone else in the meantime, rather than
  // creating an owner and silently leaving it unlinked.
  let node = null;
  if (masterNodeId) {
    node = await db.prepare(`SELECT id, owner_id FROM master_nodes WHERE id = ?`).get(masterNodeId);
    if (!node) {
      return res.status(400).json({ ok: false, error: 'This QR code was not recognized. You can still create your account and an admin will assign hardware afterward.' });
    }
    if (node.owner_id) {
      return res.status(409).json({ ok: false, error: 'This QR code has already been linked to another account. You can still create your account and an admin will assign hardware afterward.' });
    }
  }
  ...
  const color = OWNER_SIGNUP_COLORS[Math.floor(Math.random() * OWNER_SIGNUP_COLORS.length)];
  ...
  try {
    await db.prepare(
      ...
      id, name, firstName, lastName, email, phone, country, region, province,
      cityMunicipality, barangay, street, address, cityMunicipality, geoCoordinates, color, initials,
      hashPassword(password), hashConfirmToken(confirmToken), confirmTokenExpiryIso()
    );
    await restampRowHash(db, 'farm_owners', 'id', id);
  } catch (err) {
  ...
  // Link the scanned master node to the brand-new account. Guarded with
  // "AND owner_id IS NULL" so a race between two people scanning/
  // submitting the same still-unclaimed QR at once can only ever
  // succeed for one of them -- the loser's UPDATE just affects 0 rows.
  let linkedNodeId = null;
  if (node) {
    const result = await db
      .prepare(`UPDATE master_nodes SET owner_id = ?, online = 1, last_ping = datetime('now'), linked_at = datetime('now') WHERE id = ? AND owner_id IS NULL`)
      .run(id, node.id);
    if (result.changes > 0) {
      await restampRowHash(db, 'master_nodes', 'id', node.id);
      linkedNodeId = node.id;
      await logActivity(id, 'Linked master node via QR sign-up', node.id);
    }
  }
...
router.get('/owner/dashboard', requireOwnerAuth, async (req, res) => {
  ...
  nodes.forEach(applyDerivedNodeHealth);
...
// A QR sticker's payload is the full signup URL
// ("<origin>/owner/signup?nodeId=MN-STOCK-0007"), same as what
// SuperAdminQrCodesPage prints -- but an already-signed-up owner might
// also have just the bare id (typed in, or a sticker scanned with a
// generic camera app that shows raw text). Accept either shape here so
// the "Add Master Node" scanner on this page doesn't care which kind of
// QR/text it decoded.
function extractNodeId(raw) {
  ...
  try {
    ...
    // Not a URL -- fall through and treat the raw text as the id itself.
  }
  return text;
}

// Lets an owner who is ALREADY signed in add a further master node to
// their own account by scanning its QR sticker from this page (camera
// or an uploaded photo -- see src/owner/components/AddMasterNodeModal.tsx).
// This is the "add hardware after the fact" counterpart to the
// QR-at-signup flow above: same underlying link (owner_id on
// master_nodes), just reachable from inside the portal instead of only
// at account creation. Because Admin (GET /api/nodes) and Super Admin
// (GET /superadmin/nodes) both read master_nodes.owner_id live, a node
// linked here shows up under this owner in both consoles immediately,
// with no separate step.
router.post('/owner/nodes/link', requireOwnerAuth, async (req, res) => {
  const nodeId = extractNodeId((req.body || {}).masterNodeId || (req.body || {}).qrText);
  if (!nodeId) {
    return res.status(400).json({ ok: false, error: 'Could not read a master node id from that QR code.' });
  }

  const node = await db.prepare(`SELECT id, owner_id FROM master_nodes WHERE id = ?`).get(nodeId);
  if (!node) {
    return res.status(404).json({ ok: false, error: `"${nodeId}" was not recognized. Check the sticker and try again, or ask an admin to check the device.`, reason: 'not_found' });
  }
  if (node.owner_id === req.ownerRow.id) {
    return res.status(409).json({ ok: false, error: `${node.id} is already part of your master node mesh.`, reason: 'already_yours' });
  }
  if (node.owner_id) {
    return res.status(409).json({ ok: false, error: `${node.id} is already linked to another account. Ask an admin to reassign it if that's a mistake.`, reason: 'already_linked' });
  }

  // Same race-safe guard as the sign-up path: only succeeds if the node
  // is still unclaimed at the moment this UPDATE runs.
  const result = await db
    .prepare(`UPDATE master_nodes SET owner_id = ?, online = 1, last_ping = datetime('now'), linked_at = datetime('now') WHERE id = ? AND owner_id IS NULL`)
    .run(req.ownerRow.id, node.id);
  if (result.changes === 0) {
    return res.status(409).json({ ok: false, error: `${node.id} was just claimed by another account. Ask an admin to check it.`, reason: 'already_linked' });
  }

  await restampRowHash(db, 'master_nodes', 'id', node.id);
  await logActivity(req.ownerRow.id, 'Linked master node via QR scan', node.id);

  const linked = toCamel(await db.prepare(`SELECT * FROM master_nodes WHERE id = ?`).get(node.id));
  applyDerivedNodeHealth(linked);
  linked.sensors = toCamel(await db.prepare(`SELECT * FROM piezo_sensors WHERE node_id = ?`).all(node.id));
  linked.coordinates = [linked.lat, linked.lng];
