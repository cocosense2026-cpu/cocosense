...
-- SMS delivery log, mirroring outbox_emails -- every SMS notification
-- attempt is recorded here whether or not a real SMS gateway is
-- configured (see server/sms.js), same "always log, send if
-- configured" pattern the email outbox already uses.
