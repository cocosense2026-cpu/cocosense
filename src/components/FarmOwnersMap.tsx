...
export const FarmOwnersMap: React.FC<FarmOwnersMapProps> = ({ owners, onViewOwner }) => {
  ...
  // Both themes use the same standard OSM raster tiles -- these keep
  // rendering barangay/place names, residential streets, and highway
  // shields as you zoom in and out, so admins can actually confirm which
  // barangay/road a pin sits on. CARTO's hosted dark basemap now requires
  // an API key and was rendering a giant "API KEY REQUIRED" watermark, so
  // dark mode no longer depends on it -- it reuses the OSM tiles and gets
  // its dark look from a CSS filter (see `coco-map-tiles-dark` below).
    ...
    <div className="relative isolate rounded-xl bg-[#141414] border border-[#262626] overflow-hidden shadow-2xl">
      ...
      >
        <TileLayer
          ...
          className={isDark ? 'coco-map-tiles-dark' : undefined}
        />
