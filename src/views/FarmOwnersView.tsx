...
export const FarmOwnersView: React.FC<FarmOwnersViewProps> = ({
  ...
  const exportOwnersCsv = async () => {
    ...
    // Password-protected: opens a "set password" prompt, then encrypts
    // and downloads -- see src/utils/encryptedExport.ts. Replaces the
    // old plain SHA-256-stamped download since a farm-owner CSV
    // carries the same PII as the System Backup.
    requestExport('cocosense-farm-owners.csv', csvContent, 'text/csv;charset=utf-8');
  };
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {isExportModalOpen && (
        <PasswordPromptModal
          mode="set"
          title="Protect Farm Owners Export"
          description="Choose a password to encrypt this CSV. Anyone opening the downloaded file will need it -- including you, if you import it back in later."
          busy={pwBusy}
          error={pwError}
      ...
      {isImportModalOpen && (
        <PasswordPromptModal
          ...
          description="Enter the password this Farm Owners export was protected with."
          busy={pwBusy}
          error={pwError}
          onCancel={cancelPasswordFlow}
