...
export interface MasterNode {
  ...
  sector: string;
  online: boolean;
  batteryPercent: number | null; // null until the device has ever actually reported in
  signalRssi: string | null; // e.g. -68 dBm (Strong); null until the device has ever reported in
  note: string;
  totalSensors: number;
